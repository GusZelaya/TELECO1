...
class blk(gr.basic_block):
    ...
    def general_work(self, input_items, output_items):
        ...
        # Primero se vacían bytes ya formados en el buffer interno.
        ...
        # Luego se consumen símbolos nuevos solo si hay espacio
        # para entregar los bytes que se formen.
        while consumed < len(inp) and produced < len(out):
            bits_after_next_symbol = len(self.bit_buffer) + self.k

            bytes_after_next_symbol = bits_after_next_symbol // 8
            remaining_output_space = len(out) - produced

            if bytes_after_next_symbol > remaining_output_space:
                break
